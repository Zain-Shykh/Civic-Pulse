...
| [x] | resources.requests and limits set on every container | 2 | Every container (backend, frontend, postgres, redis, plus the backend initContainer) has `resources.requests`/`.limits`; confirmed live via `kubectl get hpa` showing a real `cpu: 1%/60%`, not stuck `<unknown>/60%`. Numbers are Compose's existing limits halved for requests — an explicit starting guess, not a measurement (`docs/specs/phase-11-kubernetes-manifests.md`, Open Question 3), meant to be corrected once VPA (below) actually produces a recommendation |
| [ ] | HPA v2 with tuned behavior, plus captured `kubectl get hpa -w` output and a replicas-vs-load chart from a real load test | 4 | `k8s/base/hpa.yaml` — manifest applies live, tuned behavior verbatim from the assignment (`scaleDown` 300s / `scaleUp` 0s), real utilization number confirmed (`cpu: 1%/60%`, 2/10 replicas). **Not done:** no load test was run, so no `kubectl get hpa -w` scale-out capture and no replicas-vs-load chart exist yet — left unchecked honestly rather than counted from the manifest alone |
| [ ] | VPA in recommender mode, recommendations committed, requests updated in response, HPA/VPA conflict explained | 3 | `k8s/base/vpa.yaml` applies live in `Off` mode, correctly targets the `backend` Deployment (`kubectl get vpa`/`describe vpa`, 2026-09-25); HPA/VPA conflict explained in spec text, not just a comment (Deliverable (k)). **Not done, named explicitly, not folded into the above:** only the VPA CRDs were installed (reviewed, static YAML); the recommender/updater/admission-controller components were never installed (the official installer is an unreviewed remote script, correctly blocked by the harness's auto-mode classifier — not worked around). No recommendation was ever produced or observed, so `resources.requests` above are still an unverified guess. Closing this needs a reviewed, pinned VPA controller install or a documented manual alternative — see As-Built for full detail |
