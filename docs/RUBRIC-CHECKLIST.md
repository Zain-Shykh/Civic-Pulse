...
| [x] | .dockerignore per build context, with before/after context sizes reported | 2 | `backend/.dockerignore`, `frontend/.dockerignore`. Measured 2026-09-19: raw dir size before exclusions ≈ 23 MB (backend, incl. local `.mypy_cache`/`.pytest_cache`) and ≈ 120 MB (frontend, incl. `node_modules`); actual BuildKit context transferred after `.dockerignore` ≈ 2.9 kB combined across both images (`docker compose build --no-cache --progress=plain`). Re-checked 2026-09-24 (`docs/specs/phase-10-compose-hardening.md`) against the real post-Phase-8/9/9c tree: `backend/.dockerignore` was missing `build/` and `*.egg-info` (real untracked packaging artifacts, never `COPY`'d by the Dockerfile but still bloating the context) — added; `frontend/.dockerignore` needed no change |
| [x] | Two networks with internal: true; frontend provably cannot reach the database | 4 | `compose.yaml`. Verified live 2026-09-19: `docker compose exec frontend ping -c 3 postgres` → `ping: bad address 'postgres'`, exit code 1 (DNS resolution fails — frontend isn't on `internal`). Re-verified live 2026-09-24 against the real app, not the Phase 2 skeleton (`docs/specs/phase-10-compose-hardening.md`'s As-Built) — same result, `frontend` also confirmed unable to resolve `redis` |
...
| [x] | Healthchecks on all services with depends_on: condition: service_healthy | 2 | `compose.yaml`; verified live 2026-09-19: all four containers (`frontend`, `backend`, `postgres`, `redis`) reached `Healthy` via `docker compose up -d` / `docker compose ps`. Re-verified live 2026-09-24 against the real app (`docs/specs/phase-10-compose-hardening.md`'s As-Built) — same result |
| [x] | compose.prod.yaml uses image: ${IMAGE_TAG}, no build:, no published DB or cache port | 1 | `compose.prod.yaml`; verified via `docker compose -f compose.prod.yaml config` — no `build:` key, no ports on `postgres`/`redis`. Extended 2026-09-24 (`docs/specs/phase-10-compose-hardening.md`): backend's `TRIAGE_PROVIDER`/`GEMINI_API_KEY` are now `${VAR:?msg}` required-variable references — `docker compose config` verified to fail with a named-variable error when either is unset, and to succeed and render correctly when both are set. Closes the gap flagged since Phase 7 (`docs/IMPLEMENTATION-PLAN.md`'s Phase 10 entry) |

## H · Kubernetes — 20

| Done | Item | Marks | Evidence/file |
|---|---|---|---|
| [x] | Namespace, Deployments, StatefulSet + PVC for Postgres, ClusterIP Services, Ingress routing / and /api | 5 | `docs/specs/phase-11-kubernetes-manifests.md` As-Built, 2026-09-25: live k3d cluster, `civicpulse` Namespace, backend/frontend Deployments (2 replicas each), `postgres` StatefulSet + Bound PVC, `redis` Deployment + Bound PVC, all 4 Services `ClusterIP` (`kubectl get svc`), Ingress verified with real requests — `curl /` returned the frontend, `curl /api/stats` returned real JSON (200) through Traefik |
| [x] | ConfigMap and Secret separated; committed manifests carry placeholders only | 2 | `k8s/base/configmap.yaml` (non-secret only), `k8s/base/secret.yaml` (`POSTGRES_PASSWORD`/`GEMINI_API_KEY`/`DATABASE_URL`, all placeholder `changeme` values, never a real one committed); confirmed in-cluster via `kubectl get configmap,secret -n civicpulse`, 2026-09-25 |
| [x] | All three probes correct: liveness independent of the database, readiness dependent on it | 4 | `k8s/base/backend.yaml` — startup/liveness both `/health` (confirmed via `backend/app/routes/health.py`, does not call the DB), readiness `/ready` (calls both `db_ping`/`redis_ping`); both backend replicas passed readiness live on first boot, 2026-09-25. Frontend deliberately gets one reused probe, not three — no DB dependency to differentiate (`docs/specs/phase-11-kubernetes-manifests.md`, Deliverable (e)) |
| [x] | resources.requests and limits set on every container | 2 | Every container (backend, frontend, postgres, redis, plus the backend initContainer) has `resources.requests`/`.limits`. Backend's `cpu`/`memory` requests were originally Compose's limits halved (an explicit starting guess, `docs/specs/phase-11-kubernetes-manifests.md`, Open Question 3) — since corrected to the real VPA recommendation (`163m`/`250Mi`, superseding the guess); see the VPA row below and `docs/specs/phase-11b-failfast-and-vpa-verification.md`'s As-Built |
| [x] | HPA v2 with tuned behavior, plus captured `kubectl get hpa -w` output and a replicas-vs-load chart from a real load test | 4 | `k8s/base/hpa.yaml` — tuned behavior verbatim from the assignment (`scaleDown` 300s / `scaleUp` 0s). Real load test run as a Kubernetes `Job` (`load/k6-job.yaml`, ~241 concurrent real-distinct-IP pods) against the live Ingress. Real `kubectl get hpa -w` capture: `2 → 3` scale-out at `t=100s` (`cpu: 82%/60%`, sustained 79–89% through the load window), scale-down back to `2` at `14:44:42` after the load Job completed at `14:39:57` (~285s later, matching the tuned 300s `scaleDown` stabilization window). Full sequences and the assignment's required load-arrival-vs-capacity-arrival lag writeup are in `docs/specs/phase-11b-failfast-and-vpa-verification.md`'s As-Built, dated 2026-09-26 |
| [x] | VPA in recommender mode, recommendations committed, requests updated in response, HPA/VPA conflict explained | 3 | `k8s/base/vpa.yaml` applies live in `Off` mode, targets the `backend` Deployment; HPA/VPA conflict explained in spec text (Deliverable (k)). VPA recommender component installed via reviewed, pinned static YAML (`vpa-rbac.yaml` + `recommender-deployment.yaml` from `kubernetes/autoscaler`'s own repo, applied file-by-file — never the unreviewed `hack/vpa-up.sh`; updater/admission-controller deliberately not installed, since neither does anything under this project's `Off` mode). Real, non-empty recommendation captured via `kubectl describe vpa backend-vpa`: `Target: Cpu 163m, Memory 250Mi` (stable across two captures as sampled history grew, `Lower`/`Upper Bound` narrowing from `139m`–`14715m` to `102m`–`5875m`). `k8s/base/backend.yaml`'s `resources.requests` updated to match (`500m`/`256Mi` → `163m`/`250Mi`, committed), and the load test re-run against the corrected request — this is what produced the HPA row's real scale-out. Full command output in `docs/specs/phase-11b-failfast-and-vpa-verification.md`'s As-Built, dated 2026-09-26 |
...
| [x] | Frontend cannot reach the database (network segmentation proven) | −8 | Same live proof as the G-category network row above: `ping postgres` from `frontend` fails with `bad address`; re-verified 2026-09-24 against the real app (`docs/specs/phase-10-compose-hardening.md`) |
